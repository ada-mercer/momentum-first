...
Reads `manuscript/metadata/manuscript-status.yml` (the single source of truth) and writes a
Markdown table to `manuscript/backmatter/_manuscript-status-table.md`, which the rendered
page `manuscript/backmatter/manuscript-status.qmd` includes. Keeping the generated table in
a separate `_`-prefixed partial means Quarto never treats it as its own chapter,
and the qmd stays a thin wrapper.

Run manually with `.venv/bin/python tooling/scripts/build_manuscript_status.py`,
or let it run automatically at Quarto pre-render (wired in `_quarto.yml`).
"""
...
LEDGER = ROOT / "manuscript" / "metadata" / "manuscript-status.yml"
OUTPUT = ROOT / "manuscript" / "backmatter" / "_manuscript-status-table.md"
...
def main() -> int:
    ...
    lines: list[str] = [
        "<!-- GENERATED FILE — do not edit. Source: manuscript/metadata/manuscript-status.yml",
        "     Regenerate with: .venv/bin/python tooling/scripts/build_manuscript_status.py -->",
        "",
