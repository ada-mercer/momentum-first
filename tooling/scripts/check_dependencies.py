...
def check_python_imports(modules: list[str], *, label: str) -> None:
    ...
    if code != 0:
        raise CheckFailure(
            ...
            f"Install the needed dependencies, e.g. `python3 -m pip install -r tooling/ci/requirements-minimal.txt` "
            f"for minimal checks or `python3 -m pip install -r tooling/ci/requirements.txt` for full checks."
        )
...
def check_r_packages() -> None:
    ...
    proc = subprocess.run(
        [
            "Rscript",
            "-e",
            'dir.create(path.expand(Sys.getenv("R_LIBS_USER")), recursive=TRUE, showWarnings=FALSE); .libPaths(c(path.expand(Sys.getenv("R_LIBS_USER")), .libPaths())); pkgs <- c("knitr","rmarkdown","ggplot2","svglite","ragg"); miss <- pkgs[!vapply(pkgs, requireNamespace, logical(1), quietly=TRUE)]; if (length(miss)) { message(paste(miss, collapse=",")); quit(status=1) } else { cat(paste(pkgs, collapse=","), "\n") }',
        ],
        capture_output=True,
        text=True,
        env=env,
